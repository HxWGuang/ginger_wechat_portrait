...
两种模式：
  - smart_sample：分层采样，默认 350 条，保证时间覆盖均匀
  - full_sample：  全量模式，返回所有过滤后消息（上限 3000 条防溢出）
...
def full_sample(df: pd.DataFrame, max_n: int = 3000) -> List[str]:
    ...
    if len(df) > max_n:
        sampled = df.sort_values('_score', ascending=False)['content'].head(max_n).tolist()
    else:
        sampled = df['content'].tolist()
